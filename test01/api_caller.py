...
# 配置：原始Java项目根目录（可根据实际情况修改，或通过命令行参数传入）
JAVA_PROJECT_ROOT = r"D:\MyProjects\GraduationProject\SmartTestGen\test01\TheAlgorithms-Java"
...
def call_generate_test_code(java_project_root: str = JAVA_PROJECT_ROOT):
    """调用生成测试代码接口"""
    logger.info("Step 3: Calling Generate Test Code API")
    logger.info(f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"Java Project Root: {java_project_root}")
    ...
    for i in range(1, total_rows + 1):
        ...
        if gen_result["success"]:
            ...
            # 关键修改：传入java_project_root，编译时包含原始类文件
            compile_result = CompilationUtil.compile_test_code(
                ...
                test_code=test_code,
                java_project_root=java_project_root  # 新增参数
            )
...
def call_fix_compilation_error(java_project_root: str = JAVA_PROJECT_ROOT):
    """调用修复编译错误接口"""
...
def main():
    ...
    parser.add_argument(
        "--java-root",
        default=JAVA_PROJECT_ROOT,
        help="Path to Java project root (contains src/main/java)"
    )
    ...
    logger.info(f"Java Project Root: {java_root}")
    logger.info(f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info("=" * 70)
    ...
    if args.step == "generate" or args.step == "all":
        if not call_generate_test_code(java_project_root=java_root):
            logger.error("Generate step failed")
            return 1

    if args.step == "fix" or args.step == "all":
        if not call_fix_compilation_error(java_project_root=java_root):
